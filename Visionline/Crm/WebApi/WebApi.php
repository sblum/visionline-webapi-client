...
class WebApi
{
    ...
    public function queryResponsibleContacts($type, $which)
    {
        ...
        if (\is_int($which)) {
            return $this->queryResponsibleContacts($type, [$which]);
        } elseif ($which instanceof QueryResult) {
            return $this->queryResponsibleContacts($type, [$which]);
            ...
            foreach ($which as $entry) {
                ...
                } elseif ($entry instanceof QueryResult) {
        ...
        $getIds = $this->cacheRead($type, $results, [self::FieldKeyResponsibleContacts]);

        // Transform cached results
        foreach ($results as $id => $value) {
            if (\is_array($value)) {
                $results[$id] = $value[self::FieldKeyResponsibleContacts];
        ...
        if (\count($getIds) > 0) {
            ...
            foreach ($queryResults as $queryResult) {
                if (!isset($queryResultsPerId[$queryResult->relatedTo])) {
                    $queryResultsPerId[$queryResult->relatedTo] = [];
                }
            ...
            foreach ($queryResultsPerId as $id => $queryResults) {
                ...
                $this->cacheWrite($type, $id, $results[$id]->lastModified, [self::FieldKeyResponsibleContacts => $queryResults]);
    ...
    public function queryWebsiteDocuments($type, $which, $max = null, $filterByExtension = null, $filterByDokumentart = null)
    {
        ...
        if (\is_int($which)) {
            return $this->queryWebsiteDocuments($type, [$which], $max, $filterByExtension, $filterByDokumentart);
        } elseif ($which instanceof QueryResult) {
            return $this->queryWebsiteDocuments($type, [$which], $max, $filterByExtension, $filterByDokumentart);
        } elseif (\is_array($which)) {
            // Initialize $results
        ...
        if (!$this->cacheWebsiteDocuments) {
            ...
            $syntheticFieldIdentifier = self::FieldKeyWebsiteDocumentsPrefix.\sha1(\serialize($queryArgs));

            // If cache is available, read cache entries where possible
            $getIds = $this->cacheRead($type, $results, [$syntheticFieldIdentifier]);
            ...
            if (\count($getIds) > 0) {
                ...
                foreach ($queryResultsPerId as $id => $queryResults) {
                    // If cache is available, write cache entry
                    $this->cacheWrite($type, $id, $results[$id]->lastModified, [$syntheticFieldIdentifier => $queryResults]);
    ...
    {
        if (\is_int($which)) {
            return $this->queryKontakte($type, [$which], $max, $filterByRole);
        } elseif ($which instanceof QueryResult) {
            return $this->queryKontakte($type, [$which], $max, $filterByRole);
        } elseif (\is_array($which)) {
            $ids = [];
    ...
    {
        if (\is_int($which)) {
            return $this->queryObjekte($type, [$which], $max, $filterByRole);
        } elseif ($which instanceof QueryResult) {
            return $this->queryObjekte($type, [$which], $max, $filterByRole);
        } elseif (\is_array($which)) {
            $ids = [];
    ...
    {
        if (\is_int($which)) {
            return $this->queryProjekte($type, [$which], $max, $filterByRole);
        } elseif ($which instanceof QueryResult) {
            return $this->queryProjekte($type, [$which], $max, $filterByRole);
        } elseif (\is_array($which)) {
            $ids = [];
    ...
    public function queryContactImages($which)
    {
        ...
        if (\is_int($which)) {
            return $this->queryContactImages([$which]);
        } elseif ($which instanceof QueryResult) {
            return $this->queryContactImages([$which]);
        } elseif (\is_array($which)) {
            // Initialize $results
        ...
        // If cache is available, read cache entries where possible
        $getIds = $this->cacheRead(EntityType::Kontakt, $results, [self::FieldKeyContactImages]);

        // Transform cached results
        foreach ($results as $id => $value) {
            if (\is_array($value)) {
                $results[$id] = $value[self::FieldKeyContactImages];
        ...
        if (\count($getIds) > 0) {
            ...
            foreach ($queryResultsPerId as $id => $queryResults) {
                // If cache is available, write cache entry
                $this->cacheWrite(EntityType::Kontakt, $id, $results[$id]->lastModified, [self::FieldKeyContactImages => $queryResults]);
     ...
     * @see QueryResult
     */
    public function get($type, array $which, array $fields, array $idFields = [])
    {
        $results = [];

        if (\is_int($which)) {
            return $this->get($type, [$which], $fields, $idFields);
        } elseif ($which instanceof QueryResult) {
            return $this->get($type, [$which], $fields, $idFields);
        } elseif (\is_array($which)) {
            // Initialize $results
     ...
     */
    private function cacheWrite($type, $id, $lastModified, $fields, $idFields = [])
    {
        ...
        if (isset($this->cache)) {
            ...
            if (null != $oldCacheEntry && $oldCacheEntry->lastModified >= $cacheEntry->lastModified) {
        ...
        if (isset($this->cache)) {
            foreach ($results as $result) {
                ...
                if (null != $cacheEntry) {
                    ...
                    if (null == $result->lastModified || $cacheEntry->lastModified < $result->lastModified) {
    ...
    {
        if (\is_int($contacts)) {
            return $this->getInterests([$contacts], $filterByStatus);
        } elseif ($contacts instanceof QueryResult) {
            return $this->getInterests([$contacts], $filterByStatus);
    ...
    {
        if (\is_int($enquiries)) {
            $this->getEnquiries([$enquiries]);
        } elseif ($enquiries instanceof QueryResult) {
            $this->getEnquiries([$enquiries]);
        } elseif (\is_array($enquiries)) {
            $ids = [];
